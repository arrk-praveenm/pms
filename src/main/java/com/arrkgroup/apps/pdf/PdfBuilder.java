...
public class PdfBuilder extends AbstractITextPdfView {
	...
	protected void buildPdfDocument(Map<String, Object> model, Document doc,
		...
		/*
		 * new
		 * Paragraph("Performance Assessment").setAlignment(Element.ALIGN_CENTER
		 * )
		 */
		...
		managerSign.add("Manager : ______________________" + "       "
		...
		List assesseinfo = (List) model.get("assesseinfo");
...

		{
			...
			while (assesseinfoiterator.hasNext()) {
				...
				assesseesAssessor = (AssesseesAssessor) assesseinfoiterator
						.next();

				List assesseobjectives = (List) model.get("pdfdetailsview");

				ListIterator assessedetailiterator = assesseobjectives
						.listIterator();

				// System.out.println(
				// "assessee id is   "+assesseesAssessor.getId());
				doc.add(new Paragraph(""));

				Font fontForProjectAndRoleTitle = FontFactory
				...
				para_project.add("Project : "
						+ assesseesAssessor.getProjectId().getProject_name());
				...
				para_role.add("Role : "
						+ assesseesAssessor.getRoleId().getTitle());
				...
				paraAssessor.add("Assessor : "
						+ assesseesAssessor.getAssessorId().getFullname());
				...
				table.setWidths(new float[] { 2.7f, 5.5f, 2.4f, 1.5f, 1.5f,
						3.5f, 2.2f, 2.2f, 3.5f, 1.6f });
...

				{
					...
					pdftableview objectives = (pdftableview) assessedetailiterator
							.next();

					System.out.println("objective id " + objectives.getId()
							+ "  assesseesAssessor id "
							+ assesseesAssessor.getId());
					...
					if (objectives.getId() == assesseesAssessor.getId()) {
						...
						table.addCell(String.valueOf(objectives
								.getSelf_rating()));
				...
				for (int i = 1; allSectionAssessmentScoreIterator.hasNext(); i++) {
					...
					List<SectionConsolidatedBean> listSectionConsolidatedBean = (List) allSectionAssessmentScoreIterator
							.next();
					// System.out.println("summary rating individual sizeee"+
					// listSectionConsolidatedBean.size());

					ListIterator list = listSectionConsolidatedBean
							.listIterator();
					...
					for (int j = 1; list.hasNext(); j++) {
						...
						if (sectionConsolidatedBean.getAssesseassessorid() == assesseesAssessor
								.getId()) {

							summarytable.addCell(sectionConsolidatedBean
									.getSection());

							if (showFlag) {
								summarytable.addCell(String
										.valueOf(sectionConsolidatedBean
												.getSection_point()));
							} else {
							...
							for (Section section : allSections) {
								...
								if (section.getId() == sectionConsolidatedBean
										.getId()) {
									final_weightage_role = final_weightage_role
											+ (section.getSection_weightage() * sectionConsolidatedBean
													.getSection_point());
								}
				...
				if (showFlag) {
					table1.addCell(String.valueOf((float) Math
							.round(final_weightage_role * 100) / 100));
				} else {
	...
	public Document summaryrating(Document doc,boolean showFlag)throws Exception
 {
		...
		for (int i = 1; allSectionAssessmentScoreIterator.hasNext(); i++) {
			...
			for (int j = 1; list.hasNext(); j++) {
				...
				consolidatedBean.setSection(sectionConsolidatedBean
						.getSection());
				consolidatedBean.setSection_point(consolidatedBean
						.getSection_point()
						+ sectionConsolidatedBean.getSection_point());
		...
		for (SectionConsolidatedBean sectionConsolidatedBean : conList) {
			...
			if (averageSection.get(sectionConsolidatedBean.getSection()) != null) {
				...
				averageSection.put(sectionConsolidatedBean.getSection(),
						sectionConsolidatedBean.getSection_point());
		...
		System.out.println("************************  map enries for  calculate final point****");

		Set<String> keyname= averageSection.keySet();

		for (String section : keyname) {

			System.out.println("key are "+section);
			System.out.println("average value for that section is "+averageSection.get(section) );
		...
		for (Entry<String, Float> entry : averageSection.entrySet()) {
			...
			// System.out.println("after addtion"+entry.getKey() + "/" +
			// entry.getValue());
			...
			if (showFlag) {
				...
				table.addCell(String.valueOf(String.valueOf((float) Math
						.round(entry.getValue() * 100) / 100)));
			} else {
				table.addCell(String.valueOf(0));
			}
	...
	public boolean show(List assesseinfo) {
		...
		while (assesseinfoiterator.hasNext()) {
			...
			if (assesseesAssessor.getStatus().equalsIgnoreCase("closed")
					|| assesseesAssessor.getStatus().equalsIgnoreCase("agree")
					|| assesseesAssessor.getStatus().equalsIgnoreCase(
							"assessementcompleted")) {
				showFlag = true;
